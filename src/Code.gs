...
/**
 * Safe test: sample price lookups only (no spreadsheet writes, no email).
 * Run locally: npm run run:test
 */
function testGetPriceSample() {
  const samples = [
    { id: "SGLN", type: "ETF", currency: "GBX", name: "iShares Physical Gold" }
  ];
  const results = [];
  samples.forEach(s => {
    const r = getPriceWithSource(s.id, s.type, s.currency, s.name);
    console.log(`[${s.name}] price=${r.price} source=${r.source}`);
    results.push({ name: s.name, price: r.price, source: r.source });
  });
  return JSON.stringify(results);
}

/**
 * Dry run: reads Config, fetches prices, logs totals — no sheet writes or email.
 * Run locally: npm run run:dry
 */
function dryRunPortfolioUpdate() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const configSheet = ss.getSheetByName("Config");
  if (!configSheet) {
    console.log("ERROR: Config sheet not found");
    return "Config sheet not found";
  }
  const lastRowConfig = configSheet.getLastRow();
  if (lastRowConfig < 2) return "No config rows";

  const data = configSheet.getRange(2, 1, lastRowConfig - 1, 7).getValues();
  console.log(`--- DRY RUN (${SCRIPT_VERSION}) | ${data.length} config rows ---`);

  let isaTotal = 0;
  let penTotal = 0;

  data.forEach((row, index) => {
    const platform = row[0] ? row[0].toString().trim() : "Unknown";
    const account = row[1] ? row[1].toString().trim() : "Unknown";
    const name = row[2] ? row[2].toString() : "Unknown";
    const id = row[3] ? row[3].toString().trim() : "";
    const type = row[4] ? row[4].toString().trim() : "";
    let units = parseFloat(row[5]) || 0;
    const currency = row[6] ? row[6].toString().trim().toUpperCase() : "GBP";
    ...
    if (type.toUpperCase() === "MANUAL") {
      ...
      const priceResult = getPriceWithSource(id, type, currency, name);
      price = isNaN(priceResult.price) ? 0 : priceResult.price;
      source = priceResult.source;
    }

    const value = price * units;
    if (account.toUpperCase().includes("ISA")) isaTotal += value;
    else penTotal += value;

    console.log(`Row ${index + 2}: [${platform}] ${name} | £${price.toFixed(4)} x ${units} = £${value.toFixed(2)} | ${source}`);
    Utilities.sleep(200);
  });

  const grandTotal = isaTotal + penTotal;
  console.log(`--- DRY RUN COMPLETE | ISA: £${isaTotal.toFixed(2)} | PEN: £${penTotal.toFixed(2)} | TOTAL: £${grandTotal.toFixed(2)} ---`);
  return `Dry run OK — total £${grandTotal.toFixed(2)}`;
}
